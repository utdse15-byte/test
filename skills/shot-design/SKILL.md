---
name: shot-design
description: 分镜与镜头语言工艺——景别(远/全/中/近/特写)、运镜(推拉摇移跟升降甩)、180° 轴线、动接动 cut-on-action、平均镜长 4–6s、每 3–5s 一个有意义的变化、景别多样性,以及业余分镜的通病目录。把镜头设计映射到 Manju 的 /storyboard 分镜表与 shots 的 camera 字段。触发词:分镜、镜头、景别、运镜、camera、shot list、轴线、180、跳切。
...
镜头语言是「专业 vs 业余」肉眼可见的分水岭。你的活:给每个节拍选对景别与运镜、守住轴线、让镜长和变化有节奏,并把它写进 `shots/*.yaml` 的 `camera` 与 `/storyboard` 分镜表。
...
- **单人口播** → **忌一镜到底**:每 3–5 秒切景别(中→近→特写)或插空镜,否则观众流失。
- **跨镜连续动作** → 守 **180° 轴线**(别翻轴),保持视线方向一致;道具/人物位置连续。

镜长基线:**平均镜长 4–6 秒,每 3–5 秒一个有意义的变化**(观众约需 3 秒吸收,过 5 秒注意力衰减)。[*每次运镜 0.5–1.5s、每 30s 换节奏属创作者经验值,非官方标准。*]
...
- 分镜头脚本六要素:**景别 · 内容 · 台词 · 时长 · 运镜 · 道具**——正好对应 `/storyboard` 分镜表的列。

## BEFORE / AFTER(业余 → 专业,附 WHY)

**例 1 — 一镜到底的口播**
- BEFORE:`S001 wide, 无运镜, 30s` 一个全景把整段说完。
- AFTER:切成 `S001 medium 4s → S002 close_up 3s(讲重点)→ S003 拉 wide 3s`,插一枚空镜。
- WHY:单一景别 + 超长镜必掉留存;景别多样性 + 3–5s 变化把注意力续住(呼应 `narrative-pacing`)。
...
[ ] 一场戏里景别有变化,没有一镜到底的口播?
[ ] 平均镜长 4–6s,每 3–5s 一个有意义变化?(逐镜看 duration)
...
1. 给一个 30s 单景别口播,是否被切成景别有变化、每 3–5s 一变的多镜?
