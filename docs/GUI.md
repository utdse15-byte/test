...
- **首页先给一个决定，再展开工程细节**：cockpit 只读汇聚现有创作、镜头和成片
  owner，首层固定显示“现在最值得做”、三条制作旅程、待我处理、正在运行和最近成果。
  原有镜头卡、构建、时间线、任务、事件和版本没有删除，而是进入“项目详情与高级操作”；
  默认折叠并且只有用户展开后才渲染镜头卡、读取时间线/提案并做隐藏构建试算。
  `common.js` 继续写 `manju-last-<identity>`，首页把“继续上次工作”作为安静的次要入口；
  新 take 未阅数仍来自同一本地快照，任何旅程和首页排序都不是项目真相。
...
所有主要工作台页面和项目工作区都提供同一个“快速前往”入口。它是现有导航、
镜头索引和最近项目的只读发现层，不是新的命令执行系统。
...
镜头搜索会根据查询意图保持上下文：普通查询进入镜头实验室，包含“审片/评价”
时进入 Review Theater，包含“导入/入库”时进入批量入库。它不会直接构建、生成、
重做、选片、审批、确认/执行提案或 Picture Lock。

项目索引只在首次打开或过期后按需读取；读取失败时，页面导航仍可使用，界面会
明确说明镜头和最近项目暂时无法更新，并在下一次打开时重试。该索引只存在于页面
内存，不是项目真相、构建输入或缓存身份。

## 帮助与支持（F1）

所有主要页面和首次项目工作区共用一个只读帮助入口。按 `F1`，或在“快速前往”中搜索
“帮助”，可以查看：
...
- 产品不上传使用遥测的边界；
- 全局快捷键；
- `manju doctor`、`manju support-bundle` 和 Windows App 日志位置。

帮助 API 只返回有界、无密钥的产品快照，不包含项目绝对路径、主机名、环境变量、Provider
manifest、媒体或凭据。更深入的诊断必须由用户显式运行；生成支持包不等于发送。详细数据
边界见 `docs/LOCAL_DATA_AND_PRIVACY.md`。

所有 GUI HTML 还统一发送保守的浏览器权限策略，拒绝摄像头、麦克风、定位、支付、USB、
串口和蓝牙等工作台不需要的环境权限。Provider 网络能力属于 Python 引擎的显式执行边界，
不是浏览器权限。
