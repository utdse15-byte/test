...
`manju models workbench --output my-workbench.html` 创建可独立携带的页面；完整交付包根目录也提供 OPEN_MODEL_WORKBENCH.html。
普通浏览器双击打开即可使用。受企业策略限制的机器可能禁止 file://，这不是项目能够绕过的权限；保留 CLI 作为独立入口。

先写任务，再选择素材用途并加文件，然后检查适配、选择具体模式并阅读告警，最后下载任务或完整素材包。
从工程只读导出：`manju models from-shot 雨夜.manju 镜头编号 --output shot-import.json`，然后在页面导入。
原控制图谱保留于只读上下文，程序对角色初始分类不替代人工确认。远程参考不会自动下载，未能绑定的参考会明确列出。

本页面使用 CSP 禁止网络连接，无第三方脚本。流式哈希不依赖 secure-context 的 WebCrypto。
由于本轮沙箱禁止 file:// 导航，已实际验的是隔离文档中的原字节脚本、文件选择、下载和 Python 复核，不宣称完成 Windows 文件双击与 localStorage 验收。
