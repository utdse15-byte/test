...
def data(tmp_path):
    ...
    (repo/'tools/model_workbench.html').write_text('test page');(repo/'src/manju/ide.py').write_text('source')
    (ev/'regression.rc').write_text('0');(ev/'regression.xml').write_text('<testsuite><testcase classname="test" name="real"/></testsuite>')
...
def test_missing_actual_step_blocks_publish(data,key):
    repo,ev=data;p=ev/'installed-ide/RESULT.json';r=json.loads(p.read_text());r[key]=False;put(p,r)
...
def test_gate_rejects_stale_or_unfinished(data,case):
    repo,ev=data;p=ev/'installed-ide/RESULT.json';r=json.loads(p.read_text())
    if case=='unfinished':(ev/'regression.rc').write_text('1')
    elif case=='empty':(ev/'regression.xml').write_text('<testsuite/>')
    elif case=='failed':(ev/'regression.xml').write_text('<testsuite><testcase><failure/></testcase></testsuite>')
    elif case=='duplicate':(ev/'regression.xml').write_text('<testsuite><testcase name="x"/><testcase name="x"/></testsuite>')
    elif case=='changed-source':(repo/'tools/model_workbench.html').write_text('changed')
...
def test_root_ide_entry_has_no_autorun_or_install(tmp_path):
    ...
    j=json.loads((tmp_path/'MANJU.code-workspace').read_text())
