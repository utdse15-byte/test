...
from .story import Story, brief_status, assert_settable_story
...
预览后可执行：
    ...
    python -m manju models ide-story-return . --expected-preview <刚才的preview_sha256> --output ../STORY_RETURN.json

网页会比较原稿/本地/候选，只接回用户勾选的无冲突故事字段，原素材和未选正文保持。结构默认不选并整组检查；没有故事基线的新建使用旧全本导入。此文件不含媒体或EDIT文字，excluded_edit_fields列明未包含项。未接受的JSON须单独保留，不随收工包缓冲保存。不改基线来消除冲突。
"""
...
_assert_settable_story = assert_settable_story
...
def export_story_return(workspace: Path, output: Path, *, expected_preview: str) -> dict:
    """Export a selective STORY-only return; explicitly list excluded EDIT fields."""
    from .story_return import create_return
    output = Path(output)
    if output.exists() or output.is_symlink():
        raise AuthoringError('use a new return filename; never overwrite')
    with _load(workspace) as (root, session, desk, _, studio, edit, story, snapshots):
        report = _preview(session, desk, studio, edit, story)
        if report['preview_sha256'] != expected_preview:
            raise AuthoringError('stale preview; preview again before exporting')
        old = getattr(desk, 'story', None)
        if old is None or story is None:
            raise AuthoringError('selective return requires an existing story baseline')
        packet = create_return(old, story)
        output.parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(prefix='.manju-story-return-', dir=output.parent) as td:
            staged = Path(td) / 'STORY_RETURN.json'; staged.write_bytes(canonical(packet))
            for name, expected in {**session['immutable_files'], **snapshots}.items():
                if file_digest(_file(root, name)) != expected:
                    raise AuthoringError('workspace changed during export; nothing published')
            _publish_new(staged, output)
    return {'ok': True, 'schema_id': packet['schema_id'], 'output': str(output),
            'sha256': file_digest(output), 'preview_sha256': expected_preview,
            'excluded_edit_fields': report['changed_fields'], 'media_included': False,
            'source_modified': False, 'automatic_execution': False}
