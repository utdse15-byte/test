...
$('exchange-import-json').addEventListener('change',exchangeHandled(async event=>{
 const file=event.target.files[0];if(!file)return;require(!studioBusy(),'其他文件操作仍在进行');exchangeState.busy=true;exchangeClearPreview();exchangeState.edit=null;$('exchange-preview').disabled=true;$('exchange-import-texts').disabled=true;$('exchange-save-returned').disabled=true;$('exchange-import-status').textContent='正在核验新材料，上一份改稿已释放；工作区不变。';const serial=exchangeState.serial;
 try{require(file.size<=2*1024*1024,'JSON 最大 2 MiB');const data=new TextDecoder('utf-8',{fatal:true}).decode(await file.arrayBuffer()),edit=await normalizeExchange(strictJSON(data));require(serial===exchangeState.serial,'读取期间材料选择变化，请重试');exchangeState.edit=edit;$('exchange-preview').disabled=false;$('exchange-import-texts').disabled=false;$('exchange-import-status').textContent=`已加载 ${file.name}，${Object.keys(edit.base).length} 个字段。未改变工作；可以再选择修改过的 TXT，或直接预览 JSON 中的 values。`;}
