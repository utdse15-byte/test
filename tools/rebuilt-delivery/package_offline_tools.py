...
def build(repo:Path,output:Path):
    ...
    stage=version['stage']
    if stage not in {'R7','R8','R9','R10','R11','R12','R13','R14','R15','R16'}:raise ValueError('This compact toolkit requires R7 or later')
...
'''
    ...
    payload={
      ...
      **({'INTERCHANGE_GUIDE.md':(repo/f'REPORTS/continuation/{stage}_INTERCHANGE_MATRIX.md').read_bytes()} if (repo/f'REPORTS/continuation/{stage}_INTERCHANGE_MATRIX.md').is_file() else {}),
