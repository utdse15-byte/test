...
PREFIX='MANJU_FINAL_R16_11'
VERSION='0.2.0+r16.11'
...
def evidence_gate(repo,evidence):
    ...
    reports=[json.loads((evidence/p/'RESULT.json').read_text(encoding='utf-8')) for p in ('source-final','installed-final')]
    required=('ok','actual_browser_download','actual_cli_roundtrip','three_way_report_parity',
              'local_later_fields_preserved','only_selected_story_fields_changed','ending_field_unchanged',
              'old_briefs_immutable','inner_studio_unchanged','source_backup_unchanged','undo_tested',
              'stale_preview_refused','already_applied_not_repeated','fresh_checkout_reopen_identical',
              'legacy_checkout_reexport_identical')
    for r in reports:
    ...
    a,b=reports
    for key in ('version','input_sha256','media_sha256','unchanged_media_files','browser_checkout_sha256','high_resolution_png_preserved','packet_sha256'):
...
def build(repo,destination,evidence,example):
    ...
    head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=repo,text=True,encoding='utf-8',errors='replace').strip()
    def examples(root):
        target=root/'EXAMPLES/SELECTIVE';target.mkdir(parents=True)
        for n in ('CHECKOUT_FOR_AI.zip','LOCAL_LATER_CHECKOUT.zip','STORY_RETURN.json','SELECTED_CHECKOUT.zip','READ_CHANGES.html'):
            shutil.copy2(evidence/'source-final'/n,target/n)
        story=root/'EXAMPLES/STORY';story.mkdir()
        shutil.copy2(example,story/'STORY_CHECKOUT.zip')
        (target/'READ_ME.txt').write_text('虚构故事与真实合成素材，不是商业模型作品。先恢复LOCAL_LATER_CHECKOUT.zip，再打开STORY_RETURN.json；只接回未冲突动作，保留用户后来写的对白。SELECTED_CHECKOUT.zip是测试结果，CHECKOUT_FOR_AI.zip是原导出基线。ZIP不需解压。\n待处理JSON单独保存，不在收工包中；已接回故事随原收工格式保存。原媒体和旧历史保持。\n',encoding='utf-8')
        for n in ('README.md','VALIDATION.md','PROJECT_REVIEW.md'):
            shutil.copy2(repo/'REPORTS/final_selective'/n,root/n)
        shutil.copy2(repo/'REPORTS/final_selective/README.md',root/'READ_FIRST.md')
    ...
    with tempfile.TemporaryDirectory(prefix='manju-ide-publish-',dir=destination.parent) as td:
        temp=Path(td);raw=temp/'core.zip';receipt=core.build(repo,raw,evidence,maintenance_reports=repo/'REPORTS/final_selective')
        ...
        for p in repo.glob('TASK_重建选择性接回_*.md'):shutil.copy2(p,fullroot/'HANDOFF'/p.name)
        ...
        (startroot/'NO_SOURCE_HERE.txt').write_text('这是免安装网页上手包，不包含源码、Git历史或CLI。用本地IDE接管完整软件请取 MANJU_FINAL_R16_11_FULL.zip。\n',encoding='utf-8')
        ...
        recovered=json.loads(subprocess.check_output([sys.executable,str(fresh/'APP/VERIFY_PACKAGE.py'),'--git'],text=True,encoding='utf-8',errors='replace',timeout=180))
        ...
        boot=json.loads(subprocess.check_output([sys.executable,str(fresh/'APP/source/tools/ai_bootstrap.py'),'--json'],cwd=temp,text=True,encoding='utf-8',errors='replace',timeout=30))
