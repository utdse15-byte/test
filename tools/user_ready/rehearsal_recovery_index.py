"""Read-only recovery index real-media rehearsal.
...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            page.locator('#shot-id').fill('S01_夜雨')
            ...
            before_index={p.relative_to(root/'recovery-data').as_posix():sha(p) for p in (root/'recovery-data').rglob('*') if p.is_file()}
            page.locator('#shot-id').fill('S02_白昼')
            page.locator('#prompt').fill('另一镜头，晴天窗边。不能覆盖第一份恢复点。')
            page.locator('#continue-save').click();expect(page.locator('#continue-status')).to_contain_text('已写入并核对本机恢复点',timeout=60000)
            second=page.evaluate('ManjuContinuation.state.lastPoint');assert second['id']!=point['id']
            page.locator('#prompt').fill('正在写第三个想法，读摘要不能覆盖它。')
            draft_before_index=page.evaluate('ManjuDesk.fingerprint()')
            stored_before={p.relative_to(root/'recovery-data').as_posix():sha(p) for p in (root/'recovery-data').rglob('*') if p.is_file()}
            page.locator('#continue-list').click();page.locator('#continue-index').click()
            expect(page.locator('#continue-index-status')).to_contain_text('摘要读取完成',timeout=60000)
            page.locator('#continue-search').fill('夜雨')
            assert page.locator('#continue-choice').input_value()==point['id']
            expect(page.locator('#continue-summary')).to_contain_text('5 份素材')
            expect(page.locator('#continue-summary')).to_contain_text('含外部改稿')
            expect(page.locator('#continue-summary')).to_contain_text('含个人模板')
            assert page.evaluate('ManjuDesk.fingerprint()')==draft_before_index and page.evaluate('ManjuDesk.needsSave()')
            stored_after={p.relative_to(root/'recovery-data').as_posix():sha(p) for p in (root/'recovery-data').rglob('*') if p.is_file()}
            ...
            page.screenshot(path=str(root/'index-narrow.png'))
            assert page.evaluate('document.documentElement.scrollWidth<=innerWidth+1')
            contexts[0].close();close();start()
            recovered=fresh();recovered.locator('#continue-list').click()
            expect(recovered.locator('#continue-points')).to_be_visible(timeout=10000)
            recovered.locator('#continue-index').click();expect(recovered.locator('#continue-index-status')).to_contain_text('摘要读取完成',timeout=60000)
            recovered.locator('#continue-search').fill('夜雨')
            assert recovered.locator('#continue-choice').input_value()==point['id']
            ...
            played=recovered.evaluate('''async()=>{let n=0;for(const v of document.querySelectorAll('video')){if(v.src?.startsWith('blob:')){const before=v.currentTime;await v.play();const end=Date.now()+5000;while(v.currentTime<=before&&Date.now()<end)await new Promise(r=>setTimeout(r,40));if(v.currentTime<=before)throw Error('video not progressing');v.pause();n++;}}return n;}''')
            assert played>=2
            ...
            result={'ok':True,'version':manju.__version__,'runtime_module':manju.__file__,
                ...
                'independent_backup_not_claimed':True,'read_only_index_preserved_files_and_new_draft':True,'content_search_selected_correct_point':True,'points_indexed':2,'source_example_unchanged':True,
