...
def build(repo, destination, evidence, example):
    ...
    for name in ['source-release','installed-release','source-relink','installed-relink','source-index','installed-index']:
    ...
    if json.loads((evidence/'source-index/RESULT.json').read_text())['desk_sha256'] != json.loads((evidence/'installed-index/RESULT.json').read_text())['desk_sha256']:
        raise ValueError('Source and installed recovery index exports differ')
    ...
    with tempfile.TemporaryDirectory(prefix='manju-final-build-',dir=destination.parent) as td:
        temp=Path(td);raw=temp/'core.zip';core_receipt=core.build(repo,raw,evidence,maintenance_reports=repo/'REPORTS/recovery-index')
        core_root=temp/'core-unpacked'
        with zipfile.ZipFile(raw) as z:z.extractall(core_root)
        app=next(core_root.iterdir())
        full_root=temp/'MANJU_RECOVER';full_root.mkdir();shutil.copytree(app,full_root/'APP')
        ...
        for source in repo.glob('TASK_恢复点索引_*.md'):shutil.copy2(source,full_root/'HANDOFF'/source.name)
        for filename in ['VALIDATION.md','PROJECT_REVIEW.md']:
            shutil.copy2(repo/'REPORTS/recovery-index'/filename,full_root/filename)
        full_check=inventory(full_root,version,head)
        full=temp/'MANJU_RECOVER_FULL.zip';zip_tree(full_root,full)
        start_root=temp/'MANJU_RECOVER_START';common(repo,start_root,example)
        start_check=inventory(start_root,version,head)
        start=temp/'MANJU_RECOVER_START.zip';zip_tree(start_root,start)
        ...
        checked=module(fresh/'MANJU_RECOVER/VERIFY_DELIVERY.py','fresh_outer').verify(fresh/'MANJU_RECOVER')
        ...
        restored=json.loads(subprocess.check_output([sys.executable,str(fresh/'MANJU_RECOVER/APP/VERIFY_PACKAGE.py'),'--git'],text=True,timeout=180))
        ...
        for item in parts['parts']:
            old=parts_dir/item['name'];new=parts_dir/('MANJU_RECOVER_PART_'+str(item['index']).zfill(2)+'.zip')
        ...
        helper=temp/'MANJU_RECOVER_DOWNLOAD_HELPER.html';helper.write_text(html,encoding='utf-8')
        ...
        helper_zip=temp/'MANJU_RECOVER_DOWNLOAD_HELPER.zip';zip_tree(helper_root,helper_zip)
        ...
        (destination/'MANJU_RECOVER_VERIFIED.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
        (destination/'MANJU_RECOVER_SHA256.txt').write_text('\n'.join(p['sha256']+'  '+p['name'] for p in result['files'])+'\n',encoding='utf-8')
