...
PREFIX='MANJU_CHANGES'
VERSION='0.2.0+r16.10'
...
def evidence_gate(repo,evidence):
    ...
    required=('ok','actual_browser_download','actual_cli_roundtrip','new_story_and_text_restored','old_briefs_immutable',
              ...
              'readable_story_diff_parity','current_browser_edit_shown_not_merged','stale_comparison_refused','report_contains_full_before_after')
    for r in reports:
    ...
    a,b=reports
    for key in ('version','input_sha256','media_sha256','unchanged_media_files','browser_checkout_sha256','high_resolution_png_preserved','report_sha256'):
...
def build(repo,destination,evidence,example):
    ...
    def examples(root):
        ...
        for n in ('CHECKOUT_FOR_AI.zip','RETURNED_CHECKOUT.zip','IDE_PREVIEW.json','READ_CHANGES.html'):
        ...
        (target/'READ_ME.txt').write_text('这是脚本模拟IDE编辑的虚构故事与合成素材，不是实际Codex/Claude调用。先打开READ_CHANGES.html看导出基线与候选对照；要看当前网页与返回文件的差异，先恢复CHECKOUT_FOR_AI.zip，再打开RETURNED_CHECKOUT.zip，只读查看后可取消。\n在首页打开 CHECKOUT_FOR_AI.zip 看原稿，或打开 RETURNED_CHECKOUT.zip 看AI工作副本的候选结果；均要核验预览确认，ZIP不用解压。新结果保留旧简报，旧引用显示需要复核。\n要自己操作，安装本轮CLI后按 AI_START_HERE.md 或完整包APP/source/docs/ai/OPERATIONS.md执行。不能覆盖真实作品或把后来的网页新稿自动合并。\n',encoding='utf-8')
        for n in ('README.md','VALIDATION.md','PROJECT_REVIEW.md'):
            shutil.copy2(repo/'REPORTS/story_changes'/n,root/n)
        shutil.copy2(repo/'REPORTS/story_changes/README.md',root/'READ_FIRST.md')
    ...
    with tempfile.TemporaryDirectory(prefix='manju-ide-publish-',dir=destination.parent) as td:
        temp=Path(td);raw=temp/'core.zip';receipt=core.build(repo,raw,evidence,maintenance_reports=repo/'REPORTS/story_changes')
        ...
        for p in repo.glob('TASK_可读故事差异_*.md'):shutil.copy2(p,fullroot/'HANDOFF'/p.name)
        ...
        (startroot/'NO_SOURCE_HERE.txt').write_text('这是免安装网页上手包，不包含源码、Git历史或CLI。用本地IDE接管完整软件请取 MANJU_CHANGES_FULL.zip。\n',encoding='utf-8')
